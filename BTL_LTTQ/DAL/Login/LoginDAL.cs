...
namespace BTL_LTTQ.DAL
{
    public class LoginDAL
    {
        ...
        public DataRow GetUserInfo(string tenDangNhap, string matKhau)
        {
            try
            {
                string query = "SELECT MaTK, TenDangNhap, LoaiTaiKhoan, MaGV FROM TaiKhoan WHERE TenDangNhap = @TenDangNhap AND MatKhau = @MatKhau";
        ...
        // Phương thức kiểm tra mã tài khoản có tồn tại không
        public bool CheckAccountIdExists(string maTK)
        {
            try
            {
